...
for (let i = 0; i < AlgorithmsButtons.length; i++) {
  AlgorithmsButtons[i].addEventListener("click", function () {
    if (AlgorithmsButtons[i].value == "Affine") {
      document
        .querySelector("#appearWhenSelectAffine")
...
document.querySelector("#convert").addEventListener("click", function () {
  if (
    ...
    if (TypeSelected == "Encripte") {
      ...
      output.innerHTML = encryptAffine(
        input1.value,
        Number(typeA.value),
        Number(typeB.value)
      );
      ...
      output.innerHTML = decryptAffine(
        input1.value,
        Number(typeA.value),
        Number(typeB.value)
      );
  ...
  for (let i = 0; i < plainText.length; i++) {
    ...
    if (charCode >= 65 && charCode <= 90) {
      cipherText += String.fromCharCode(
        ((charCode - 65 + (key[keyIndex].toUpperCase().charCodeAt(0) - 65)) %
          26) +
          65
      );
      ...
      cipherText += String.fromCharCode(
        ((charCode - 97 + (key[keyIndex].toLowerCase().charCodeAt(0) - 97)) %
          26) +
          97
      );
      keyIndex = (keyIndex + 1) % key.length;
  ...
  for (let i = 0; i < cipherText.length; i++) {
    ...
    if (charCode >= 65 && charCode <= 90) {
      plainText += String.fromCharCode(
        ((charCode -
          65 -
          (key[keyIndex].toUpperCase().charCodeAt(0) - 65) +
          26) %
          26) +
          65
      );
      ...
      plainText += String.fromCharCode(
        ((charCode -
          97 -
          (key[keyIndex].toLowerCase().charCodeAt(0) - 97) +
          26) %
          26) +
          97
      );
...
function gcd(a, b) {
  ...
  // Base case: GCD is found when one of the numbers becomes zero
  if (b === 0) {
    return a;
  }

  // Recursive case: Call the gcd function with updated values
  return gcd(b, a % b);
}
const result = gcd(7, 26);
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      if (charCode >= 65 && charCode <= 90) {
        encryptedText += String.fromCharCode(
          ((a * (charCode - 65) + b) % 26) + 65
        );
      } else if (charCode >= 97 && charCode <= 122) {
        encryptedText += String.fromCharCode(
          ((a * (charCode - 97) + b) % 26) + 97
        );
      } else {
    ...
    for (let i = 0; i < encryptedText.length; i++) {
      ...
      if (charCode >= 65 && charCode <= 90) {
        decryptedText += String.fromCharCode(
          ((modInverse * (charCode - 65 - b + 26)) % 26) + 65
        );
      } else if (charCode >= 97 && charCode <= 122) {
        decryptedText += String.fromCharCode(
          ((modInverse * (charCode - 97 - b + 26)) % 26) + 97
        );
      } else {
